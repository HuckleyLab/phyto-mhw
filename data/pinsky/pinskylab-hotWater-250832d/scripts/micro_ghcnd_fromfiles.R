...
# Now gets TMIN and TMAX daily anomalies from nearest GHCND station (previously downloaded)
# Must set timeinterval=365 (daily). Use nyears=20 if mindate='2018-01-01' and maxdate='2018-12-31' were used for ghcn_download.r
#
# loc must be numeric in format: c(lon, lat)
#
# Needs additional arguments:
# ghcnd_dir: path to the directory of files with GHCND data. Files must be of the format ghcnd_latXXX_lonYYY.rds, where XXX and YYY match the loc (lon, lat) numbers exactly. Run ghcn_download.r to make these files.
#
...
micro_ghcnd_fromfiles <- function (loc = c(14.19, 55.40), timeinterval = 365, 
    ...
    rainfrac = 0.5, shore = 0, tides = matrix(data = 0, nrow = 24 * 
        timeinterval * nyears, ncol = 3), lamb = 0, IUV = 0, ghcnd_dir) 
{
    ...
    # try to get GHCN data before going through the rest of the script
    if(errors==0){
		...
		ghcn <- NULL
			...
			# find file that matches our lat/lon
		files <- list.files(ghcnd_dir)
		files <- gsub('ghcnd_lat|.rds', '', files)
		spl <- strsplit(files, '_lon')
		latlons <- sapply(spl, paste, collapse=' ')
		this <- which(paste(loc[2], loc[1], sep=' ') == latlons)		
		if(length(this)==0){
			message(paste("ERROR: Failed to find a matching file for this lat/lon in", ghcnd_dir, "\n"))
			errors <- 1	
		}
		
			# read in file
		ghcn <- readRDS(paste(ghcnd_dir, 'ghcnd_lat', loc[2], '_lon', loc[1], '.rds', sep=''))
		
			# check that it worked
		if(is.null(ghcn)){
			message(paste("ERROR: Failed to load data for this lat/lon from", ghcnd_dir, "\n"))
			errors <- 1	
		}
	}    
